...
import { ShoppingBag, X, Trash2, ShieldCheck, ShoppingCart, CreditCard, Copy, Check, User, Building2, Phone } from 'lucide-react';
...
export default function CartDrawer({
  ...
  const handleCheckout = () => {
    ...
    if (!buyerPhone.trim()) {
      setValidationError('Mohon masukkan No. Telepon / WhatsApp Anda.');
      return;
    }
    ...
    messageText += `--------------------------------------------------\n`;
    messageText += `*INFORMASI PEMESAN:*\n`;
    messageText += `• *Nama Pemesan:* ${buyerName.trim()}\n`;
    messageText += `• *Perusahaan/Toko/Pribadi:* ${buyerCompany.trim() || '-'}\n`;
    messageText += `• *No. Telepon/WA:* ${buyerPhone.trim()}\n`;
    messageText += `--------------------------------------------------\n`;
    ...
    if (paymentMethod.includes('Transfer Bank BCA')) {
      messageText += `  └ *Detail Transfer Bank BCA:*\n`;
      messageText += `    - *Bank:* BCA (Bank Central Asia)\n`;
      messageText += `    - *No. Rekening:* 0888777132\n`;
      messageText += `    - *Atas Nama:* CV. CS FIRE FIGHTER\n`;
      messageText += `    - ⚠️ *PENTING (BUKTI TRANSFER):* Mohon lampirkan atau kirimkan foto/screenshot Bukti Transfer Anda di bawah pesan ini agar pesanan dapat segera diproses.\n`;
    }
      ...
      >
        ...
        <div className="p-5 flex-grow overflow-y-auto space-y-4">
          {cart.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {/* Form Data Pemesan */}
              <div className="pt-5 border-t border-slate-200 mt-6 space-y-4 text-left">
                <div className="flex items-center gap-2 mb-1">
                  <div className="w-1.5 h-4 bg-brand-600 rounded-full"></div>
                  <h4 className="font-bold text-slate-900 font-outfit text-sm uppercase tracking-wide">
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="buyerName" className="text-[10px] font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5 select-none">
                    <User className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                    <span>Nama Lengkap <span className="text-brand-600 font-black">*</span></span>
                  </label>
                  <input
                    type="text"
                    id="buyerName"
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value.trim() && buyerPhone.trim()) {
                        setValidationError('');
                      }
                    }}
                    placeholder="Contoh: Budi Santoso"
                    className="w-full bg-slate-50 border border-slate-200 focus:border-brand-500 focus:bg-white focus:outline-none rounded-xl px-3 py-2.5 text-xs text-slate-800 font-medium transition duration-150"
                  />
                </div>

                {/* Nama Perusahaan / Toko / Pribadi Input */}
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="buyerCompany" className="text-[10px] font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5 select-none">
                    <Building2 className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                    <span>Nama Perusahaan / Toko / Pribadi <span className="text-slate-400 font-bold shrink-0">(Pribadi/Toko/Perusahaan)</span></span>
                  </label>
                  <input
                    type="text"
                    id="buyerCompany"
                    ...
                    placeholder="Contoh: CV. Mandiri Jaya / Toko Makmur / Pribadi"
                    className="w-full bg-slate-50 border border-slate-200 focus:border-brand-500 focus:bg-white focus:outline-none rounded-xl px-3 py-2.5 text-xs text-slate-800 font-medium transition duration-150"
                  />
                </div>
                ...
                {/* No. Telepon Input */}
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="buyerPhone" className="text-[10px] font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5 select-none">
                    <Phone className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                    <span>No. Telepon / WhatsApp <span className="text-brand-600 font-black">*</span></span>
                  </label>
                  <input
                    type="tel"
                    id="buyerPhone"
                    ...
                    onChange={(e) => {
                      ...
                      if (buyerName.trim() && e.target.value.trim()) {
                        setValidationError('');
                      }
                    }}
        ...
        {cart.length > 0 && (
          <div className="p-5 border-t border-slate-100 bg-slate-50 flex flex-col gap-4">
            ...
            {validationError && (
              <div className="bg-red-50 border border-red-200 text-red-800 text-[11px] px-3 py-2 rounded-xl font-bold animate-fadeIn flex items-center gap-1.5">
